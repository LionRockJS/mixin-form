import {Controller, ControllerState, ControllerMixin} from "@lionrockjs/mvc";
...
export default class ControllerMixinCaptcha extends ControllerMixin {
  ...
  static async assign_template_data(state: Map<string, any>) {
    const captcha = await state.get(this.CAPTCHA_ADAPTER).create();
    Object.assign(
      state.get((ControllerMixinView as any).TEMPLATE).data,
      captcha
    )
  }
  ...
  static async action_update(state: Map<string, any>) {
    ...
    if(!isValid){
      const client = state.get((Controller as any).STATE_CLIENT);
