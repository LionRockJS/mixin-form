...
export default class ControllerMixinCaptcha extends ControllerMixin {
    ...
    static init(state) {
        if (!state.get(this.CAPTCHA_ADAPTER))
            state.set(this.CAPTCHA_ADAPTER, this.defaultAdapter);
    }
    static async assign_template_data(state) {
        const captcha = await state.get(this.CAPTCHA_ADAPTER).create();
        Object.assign(state.get(ControllerMixinView.TEMPLATE).data, captcha);
    }
    static async action_update(state) {
        if (state.get(this.CAPTCHA_ADAPTER).checkEnabled() === false)
            return;
